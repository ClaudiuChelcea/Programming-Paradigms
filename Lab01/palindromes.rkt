...
(define (num->base n b)
 (if (= n 0) '() (append (num->base (quotient n b) b) (list (modulo n b)))))
...
(define (all-palindromes? n Bases)
  (if (null? Bases) #t
      (if (and (equal? (palindrome? (num->base n (car Bases))) #t) (all-palindromes? n (cdr Bases))) #t #f)))
