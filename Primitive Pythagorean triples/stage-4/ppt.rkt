...
(define (stream-drop stream n)
  (if (= 0 n)
      stream
      (stream-drop (stream-rest stream) (sub1 n))))

(define (get-combinations nr-elements level G H)
  (let repeat-cons [(new-list (stream-take G nr-elements)) (element-to-cons-with (stream-first (stream-drop H level))) (list-ans '())]
    (if (null? new-list)
        list-ans
...
(define (pairs G H)
  (let iter-2 [(nr-elements 1) (level 0)]
    (if (= 0 level)
        (stream-cons (cons (stream-first G) (stream-first H)) (iter-2 (add1 nr-elements) (add1 level)))
        (stream-append (get-combinations nr-elements level G H) (stream-lazy (iter-2 (add1 nr-elements) (add1 level)))))))
