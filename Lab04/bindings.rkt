...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* [(segment (get-line-segment)) (left-pct (get-start-point segment)) (right-pct (get-end-point segment))]
  (let [(x2 (car right-pct)) (x1 (car left-pct)) (y2 (cdr right-pct)) (y1 (cdr left-pct))]
...
(define (compute-perimeter points)
  (let iter [(index 1) (result 0)]
    (if (null? points)
        result
        (iter (cdr points) (+ result (distance (car points) (car (cdr points))))))))
